...
class PlantPairTest extends JUnitSuite with MockitoSugar{
  ...
  @Test def selectHet() {
    ...
    assert(resultPlant.get === pair.selectHet(criteriaList).get)
    verify(plantCrosser).selectHeterozygousOffspring(pair, criteriaList)
  }
  
  @Test def selectHom() {
    when(plantCrosser.selectHomozygousOffspring(pair, criteriaList)).thenReturn(resultPlant)
    assert(resultPlant === pair.selectHom(criteriaList))
    verify(plantCrosser).selectHomozygousOffspring(pair, criteriaList)
  }
