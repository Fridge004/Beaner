...
class CentimorganTest extends JUnitSuite with MockitoSugar {
  val tolerance = 1e-16
  ...
  def normalisingPreCombining() {
    ...
    assert(1.0 === result.alleles.values.sum)
  }

  def totalProbability(cM:Centimorgan):Double = {
    cM.alleles.values.sum
  }
